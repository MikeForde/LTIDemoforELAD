...
<html>
<head>
  <title>DEMO of Learning Tools Interoperability for ELAD - Modified from IMS orginal</title>
  <meta http-equiv="Content-Type" content="text/html; charset=UTF-8" />
</head>
<body style="font-family:sans-serif">
<?php
echo("<h1><b>LTI Consumer - e.g. imagine this is Moodle or Canvas</b></h1>\n");
echo("<p>Demo implementaton of both sides of LTI - Consumer and Provider for ELAD 2018.</p>\n");
echo("<p>Modified by Mike Forde from original IMS instructional demo.</p>\n");
echo("<hr>");
echo("<h2>Mandatory data that instructor would configure (once) to connect to LTI tool</h2>\n");
echo("<h3><i>As seen later secret is not transmitted directly but is used to make Oauth signature</i></h3>\n");
echo("<h3><i>The tool provider also has this secret and uses it to see if Oauth signature is valid</i></h3>\n");
...
require_once("ims-blti/blti_util.php");
  ...
  if ( ! $endpoint ) $endpoint = str_replace("lms.php","tool_index.php",$cur_url);
...
<?php
  ...
  echo("<hr>");
  echo("<h2>Optional data transmitted from Consumer to Provider - the tool may or may not need or use this data</h2>\n");
  echo("<h3><i>Although optional, user_id would be needed if user is to be 'remembered' by tool</i></h3>\n");
  echo("<h3><i>Any personal identifiable data can be omitted if any GDPR issues</i></h3>\n");
  ...
  echo("<hr>");
  echo("<h2>This is what a student usually sees - i.e. a course name and a button</h2>\n");
  echo("<h3><i>Use the 'Toggle Resource and Launch' Data link above to hide the sections above</i></h3>\n");
  echo("<h3><i>Fictious Course Name</i></h3>\n");
  print($content);
